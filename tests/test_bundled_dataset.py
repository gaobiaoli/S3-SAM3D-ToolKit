...
def test_bundled_s23dis_dataset():
    ...
    assert dataset.list_scenes() == ["hallway_2"]
    scene = dataset.get_scene("hallway_2")
    frame = scene.get_frame(1)
    assert frame.uuid == "c6655e566bca4de983fc3affab814587"
    point_map = frame.point_map()
    cloud = scene.reconstruct(frame_id=1, stride=16, voxel_size=None)
